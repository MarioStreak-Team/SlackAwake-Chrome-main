...
async function ready() {
  ...
  if (currentState === true) {
    const activateElement = document.querySelector(
      'div.label[data-localize="ACTIVATE"]'
    );
...
async function onCheckBoxChanged(e) {
  ...
  if (preference.permissions !== null) {
    if (preference.permissions.includes("downloads")) {
      if (target.checked) {
        const permissionGranted = await requestDownloadPermission().catch(
          (error) => {
            console.error("An error occurred:", error);
            target.checked = !target.checked;
          }
        ...
        const permissionRemoved = removeDownloadpermission().catch((error) => {
          console.error("An error occurred:", error);
          target.checked = !target.checked;
        });
